...
while (($gap_id, $aln) = each %flanking_aln) {
	...
	$inner_aln_start = $gap_start - ($aln->[0]{q_len} - $aln->[0]{q_end});
	$inner_aln_end = $gap_end + $aln->[1]{q_start};

	if ($aln->[0]{strand} eq '+') {
		$left_inner_aln_edge = $aln->[0]{t_end};
		$right_inner_aln_edge = $aln->[1]{t_start};
		...
		$left_inner_aln_edge = $aln->[1]{t_end};
		$right_inner_aln_edge = $aln->[0]{t_start};
		if ($right_inner_aln_edge - $left_inner_aln_edge < 0) {
			$inner_aln_start = $gap_start - ($aln->[0]{q_len} - $aln->[0]{q_start});
			$inner_aln_end = $gap_end + $aln->[1]{q_end};
