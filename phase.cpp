...
void Phase::usage(void) const
{
    ...
    std::cerr << "\nUsage: platanus_b phase [Options]\n"
              << "Options:\n"
...
void Phase::execSolveDBG(const unsigned long times)
{
	vector<string> singleArgOptionList = {"-t", "-tmp", "-l", "-k", "-mapper"};
	vector<string> multiArgOptionList = {"-x", "-X", "-s"};
...
void Phase::execGapClose(void)
{
	...
	vector<string> boolArgOptionList = {"-reduce_redundancy"};
