...
#ifndef SUB_BIN_PATH_DEFINED
#define SUB_BIN_PATH_DEFINED "."
#endif
...
namespace platanus
{
    ...
    enum ERROR
    {IO=0, FOPEN, TMP, FORMAT, READ, SUBBIN, KMERDIST, MAP, BASE, COVERAGE, DOUBLEHASH, CLINK, CDIR, ASSEMBLE, SCAF, SOLVE, DIVIDE, KMERDIVIDE, POLISH, GAP, MERGE};
...
// set constatnt parameter 
    struct ConstParam
    {
        ...
        static const std::string SUB_BIN_PATH;
...
// throwing object in exception
    ...
    struct FormatError : public ErrorBase
    {
        ...
        FormatError(const std::string &mes): ErrorBase(FORMAT, "Error, File format exception!!\n" + mes) {}
        ~FormatError() {}
    };
    ...
    struct ReadError : public ErrorBase
    {
        ...
        ReadError(const std::string &mes): ErrorBase(READ, "Error, Read file exception!!\n" + mes) {}
        ~ReadError() {}
    };

    struct SubBinError : public ErrorBase
    {
        SubBinError(): ErrorBase(SUBBIN, "Error, sub-executable file is not found or executed!!\n") {}
        SubBinError(const std::string &mes): ErrorBase(SUBBIN, "Error, sub-executable file " + mes + " is not found or executed!!\n") {}
        ~SubBinError() {}
    };
    ...
    struct KmerDistError : public ErrorBase
    {
        ...
        KmerDistError(const std::string &mes): ErrorBase(KMERDIST, "Error, Kmer distribution exception!!\n" + mes) {}
        ~KmerDistError() {}
    };
    ...
    struct AssembleError : public ErrorBase
    {
        AssembleError(): ErrorBase(ASSEMBLE, "Error, Assemble exception!!\nplatanus_b assemble command failed.") {}
        ~AssembleError() {}
    };
    ...
    {
        DivideError(): ErrorBase(DIVIDE, "Error, Divide exception!!\nplatanus_b divide command failed.") {}
        ~DivideError() {}
    };

    struct KmerDivideError : public ErrorBase
    {
        KmerDivideError(): ErrorBase(KMERDIVIDE, "Error, KmerDivide exception!!\nplatanus_b kmer_divide command failed.") {}
        ~KmerDivideError() {}
    };

    struct ScaffoldError : public ErrorBase
    {
        ScaffoldError(): ErrorBase(SCAF, "Error, Scaffold exception!!\nplatanus_b scaffold/solve_DBG command failed.") {}
        ~ScaffoldError() {}
    };

    struct SolveDBGError : public ErrorBase
    {
        SolveDBGError(): ErrorBase(SOLVE, "Error, SolveDBG exception!!\nplatanus_b solve_DBG command failed.") {}
        ~SolveDBGError() {}
    };

    struct PolishError : public ErrorBase
    {
        PolishError(): ErrorBase(POLISH, "Error, Polish exception!!\nplatanus_b polish command failed.") {}
        ~PolishError() {}
    };

    struct GapError : public ErrorBase
    {
        GapError(): ErrorBase(GAP, "Error, Gap_close exception!!\nplatanus_b gap_close command failed.") {}
        ~GapError() {}
    };

    struct MergeError : public ErrorBase
    {
        MergeError(): ErrorBase(MERGE, "Error, Merge exception!!\nplatanus_b merge command failed.") {}
        ~MergeError() {}
    };
