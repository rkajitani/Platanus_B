...
, contig(), contigUnlinkSet(), allLibraryMT(NULL), longReadLibraryMT(NULL)
, tagLibraryMT(NULL), targetLibraryIndex(0), coverage(), seqPool(), node()
...
void ScaffoldGraph::deleteThinEdge(const long linkThreshold)
{
    vector<long> ids;
    unsigned long long numDelete = 0;

    std::cerr << "removing thin edges (NUM_LINK < min(" << linkThreshold << ", EXPECTED_NUM_LINK))" <<  " ..." << std::endl;
    ...
    for (long nodeID = 0 ; nodeID < numNode; ++nodeID) {
        ...
        for (long edgeID = 0; edgeID < node1.numEdge; ++edgeID) {
            ...
            if (node1.edge[edgeID].numLink < std::min(linkThreshold, expectedNumLink)) {
