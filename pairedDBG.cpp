...
{
	const double COVERAGE_THRESHOLD = HETERO_COVERAGE_THRESHOLD_FACTOR * this->averageCoverage;
	...
	for (long t = 0; t < numThread; ++t) {
		...
		for (long centerNodeIndex = t; centerNodeIndex < numNode; centerNodeIndex += numThread) {
			if (calcNodeCoverage(this->node[centerNodeIndex]) < COVERAGE_THRESHOLD  || this->node[centerNodeIndex].state & (DBG_PRIMARY_BUBBLE | DBG_SECONDARY_BUBBLE))
				continue;
...
void PairedDBG::loadResultSeq(const long minSeqLength, const unsigned long long readLength, const bool trimFlag)
{
...
void PairedDBG::outputResultSeqComponent(const string filePrefix, const string &fileSuffix)
{
    ...
    for (long i = 0; i < resultSeq.size(); ++i) {
		if (!(resultSeq[i].seq.empty() || (node[i].state & SC_DEL)))
			out << resultSeq[i].component;
	}
    ...
    for (long i = 0; i < numNode; ++i) {
        ...
        while (candidate.size() > 0) {
            ...
            for (unsigned j = 1; j < candidate.size(); ++j) {
                ...
                else if (candidate[j].distance < minDistance || (candidate[j].distance == minDistance && abs(candidate[j].start) < closest)) {
                    minDistance = candidate[j].distance;
                    closest = abs(candidate[j].start);
                    minCandidateID = j;
                }
...
/*
        ...
        while (fread(&leftPosition, sizeof(platanus::Position), 1, (*allLibraryMT)[targetLibraryIndex][threadID].mappedSameContigFP)) {
			...
			long nodeIndex = id2Index(contigPositionInScaffold[contigIndex].id);
			if (node[nodeIndex].length < lengthThreshold)
				continue;
			...
			range.first = std::min(std::max(nodeLeftOffset + innerLength, 0L), node[nodeIndex].length - 1);
			range.second = std::min(std::max(nodeLeftOffset + innerLength + averageInsSize, 0L), node[nodeIndex].length - 1);
			omp_set_lock(&lock[nodeIndex]);
			for (long i = range.first; i <= range.second; ++i) {
				++(diffCoverage[nodeIndex][i]);
			}
			omp_unset_lock(&lock[nodeIndex]);

			range.first = std::min(std::max(nodeLeftOffset + insertLength - innerLength - averageInsSize + 1, 0L), node[nodeIndex].length - 1);
			range.second = std::min(std::max(nodeLeftOffset + insertLength - innerLength, 0L), node[nodeIndex].length - 1);
			omp_set_lock(&lock[nodeIndex]);
			for (long i = range.first; i <= range.second; ++i) {
				++(diffCoverage[nodeIndex][i]);
			}
...
void PairedDBG::divideNodeBasedOnCoverage(const vector<vector<unsigned> >& physicalCoverage, const vector<vector<unsigned> >& diffCoverage, const bool bubbleFlag, const long numThread)
{
	...
	const double MAX_DIFF_COVERAGE_RATE = 2.0;
