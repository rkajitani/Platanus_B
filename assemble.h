...
private:
    ...
    template <typename KMER> void outputAndAfterTreatment(const unsigned k, FILE ** readFP, FILE *contigFP, FILE *junctionFP, BruijnGraph<KMER> &graph, Counter<KMER> &counter, const unsigned long long numThread);
	template <typename KMER> void mergeContig(platanus::Contig &contig, Counter<KMER> &counter, BruijnGraph<KMER> &graph, const unsigned long long kmerLength, FILE **mergedContigFP, FILE **mergedJunctionFP);
